...
function Interface:CreateTab(name)
    ...
    function Tab:AddSection(sName)
        ...
        function Section:AddDropdown(text, options, isMulti, callback)
            ...
            -- Главный контейнер элемента (Label + Box)
            local DropFrame = Instance.new("Frame")
            DropFrame.Name = "Dropdown_" .. text
            DropFrame.Size = UDim2.new(1, 0, 0, 35) -- Фиксированная высота элемента в списке
            ...
            -- Кнопка-бокс (за которую дергаем)
            ...
            -- Тот самый плавающий список
            local Container = Instance.new("Frame")
            Container.Size = UDim2.new(1, 0, 0, 0) -- Высота меняется скриптом
            Container.Position = UDim2.new(0, -1, 1, 1) -- Сдвиг на 1 пиксель для идеальной стыковки рамок
            Container.BackgroundColor3 = Interface.Theme.ElementBG
            Container.BorderColor3 = Interface.Theme.Outline
            Container.BorderSizePixel = 1
            Container.Visible = false
            Container.ZIndex = 100 -- Всегда сверху
            Container.Parent = MainBtn

            -- Отключаем обрезку у всех родителей, чтобы список "вылетал" за пределы секции
            ...
            local function updateValueText()
                ...
                for _, opt in ipairs(options) do
                    if Dropdown.Selected[opt] then table.insert(selectedList, opt) end
                end
                local result = table.concat(selectedList, ", ")
                ValueLabel.Text = (result == "" and "None" or result)
            end
            ...
            local function toggleDropdown()
                ...
                -- Управление слоями (чтобы текущий дропдаун был выше соседних элементов)
                DropFrame.ZIndex = Dropdown.Open and 50 or 5
                SectionFrame.ZIndex = Dropdown.Open and 50 or 1
                
                if Dropdown.Open then
                    Container.Size = UDim2.new(1, 2, 0, #options * 18) -- +2 пикселя к ширине для компенсации рамок
                end
            end
            ...
            for i, optName in ipairs(options) do
                ...
                OptBtn.MouseEnter:Connect(function() 
                    if not Dropdown.Selected[optName] then 
                        OptBtn.TextColor3 = Interface.Theme.TextActive 
                    end
                end)
            ...
            local clickConn = MainBtn.MouseButton1Click:Connect(toggleDropdown)
            table.insert(Interface.Connections, clickConn)
        end
